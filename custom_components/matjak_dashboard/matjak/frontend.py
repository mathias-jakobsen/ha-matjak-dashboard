...
async def async_setup(hass: HomeAssistant, config: MJ_Config) -> None:
    ...
    await async_setup_resources(hass)
    await async_setup_theme_file(hass, config)
...
async def async_setup_resources(hass: HomeAssistant) -> None:
...
async def async_setup_theme_file(hass: HomeAssistant, config: MJ_Config) -> None:
    """ Sets up the theme file in the configured path. """
...
async def async_remove_theme_file(hass: HomeAssistant, config: MJ_Config) -> None:
    """ Removes the theme file. """
    destination_path = hass.config.path(config.themes_path, THEMES_FILE_PATH_DESTINATION)
