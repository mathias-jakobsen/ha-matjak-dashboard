...
class Areas(BaseRegistry[area_registry.AreaEntry]):
    ...
    def __init__(self, hass: HomeAssistant, config: MatjakUserConfig):
        ...
        exclude_values = config.exclude.areas
    ...
    def _sort_registry(self, registry: dict[str, area_registry.AreaEntry], config: MatjakUserConfig) -> dict[str, area_registry.AreaEntry]:
    ...
    def get_area_icon(self, area_entry: area_registry.AreaEntry) -> str:
        ...
        area_config = self.config.areas.get(area_entry.id, self.config.areas.get(area_entry.name, {}))
    ...
    def group_by_location(self) -> tuple[str, list[area_registry.AreaEntry]]:
        ...
        area_config = self.config.areas
        location_config = self.config.area_locations
